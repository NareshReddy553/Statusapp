...
class IncidentSerializer(serializers.ModelSerializer):
    ...
    def get_recipients(self, obj):
        recipient_obj = IncidentAdditionalRecipients.objects.filter(
            incident=obj, is_active=True
        )
    ...
    def get_components(self, obj):
        final_list = []
        inc_comp_obj = IncidentComponent.objects.filter(incident=obj.pk, is_active=True)
        if inc_comp_obj:
            for data_obj in inc_comp_obj:
                temp_dict = {}
                l_last_component_status = None
                if obj.status == "resolved":
                    l_last_component_status = (
                        IncidentsComponentActivitys.objects.filter(
                            incident_id=obj.pk,
                            component_id=data_obj.component.component_id,
                ...
                if data_obj.component.is_active:
                    ...
                    temp_dict[
                        "component_status"
                    ] = data_obj.component.component_status.component_status_name
                    if l_last_component_status and len(l_last_component_status) >= 2:
                        temp_dict["last_component_status"] = l_last_component_status[
                            1
                        ].component_status
                    else:
                    ...
                    temp_dict[
                        "businessunit"
                    ] = data_obj.component.businessunit.businessunit_name
    ...
    def create(self, validated_data):
        ...
        for cmp_sts in components_list:
            component_status_obj = ComponentsStatus.objects.filter(
                component_status_name=cmp_sts.get("component_status")
            ).first()
            # update the component status in component table
            update_component_obj = Components.objects.filter(
                pk=cmp_sts.get("component_id")
            ).first()
            ...
            if inc_cmp_obj:
                for inc_cmp_data in inc_cmp_obj:
                    cmp_qs = Components.objects.filter(
                        component_id=inc_cmp_data.component_id
                    ).first()
...
class IncidentsActivitySerializer(serializers.ModelSerializer):
    ...
    def get_components(self, obj):
        serializer = IncidentsComponentActivitysSerializer(
            IncidentsComponentActivitys.objects.filter(incidents_activity_id=obj.pk),
            many=True,
        )
        return serializer.data
...
class StatusPageIncidentsSerializer(serializers.ModelSerializer):
    ...
    def get_activity_history(self, obj):
        serializer = StatusPageIncidentsActivitySerializer(
            IncidentsActivity.objects.filter(incident=obj), many=True
        )
        return serializer.data
...
class SubscribersSerializer(serializers.ModelSerializer):
    ...
    def get_components(self, obj):
        component_obj = Components.objects.filter(
            component_id__in=SubcriberComponent.objects.filter(
            ...
            if l_network:
                ...
                if l_network_mail:
                    validated_data["email"] = (
                        validated_data["phone_number"] + "@" + l_network_mail
                    )
...
class ScheduledMaintanenceSerializer(serializers.ModelSerializer):
    ...
    schstartdate = serializers.DateTimeField(
        error_messages={"invalid": _("This field is required")}
    )
    schenddate = serializers.DateTimeField(
        error_messages={"invalid": _("This field is required")}
    )

    def get_components(self, obj):
        component_obj = Components.objects.filter(
            component_id__in=SchMntComponent.objects.filter(
                sch_inc=obj, businessunit=obj.businessunit, is_active=True
            ).values_list("component_id", flat=True)
        )
        serializer = ComponentsSerializer(component_obj, many=True)
        return serializer.data
    ...
    def get_recipients(self, obj):
        recipient_obj = SchMntAdditionalRecipients.objects.filter(
            sch_inc=obj, is_active=True
        )
    ...
    def update(self, instance, validated_data):
        ...
        instance.schstartdate = validated_data.get(
        ...
        sch_mnt_component_obj = SchMntComponent.objects.filter(
            sch_inc=instance, businessunit=businessunit_qs
        )
        l_components = []
        ...
        try:
            ...
            if l_components:
                for cmp_id in l_components:
                    sch_mnt_component_create.append(
                        SchMntComponent(
                            sch_inc=instance,
                            component_id=cmp_id,
                            is_active=True,
            ...
            if sch_mnt_component_create:
                sch_mnt_cmp_obj = SchMntComponent.objects.bulk_create(
                    sch_mnt_component_create
                )
            if not instance.status == "Scheduled":
                Components.objects.filter(
                    businessunit=instance.businessunit,
                    is_active=True,
                    component_id__in=l_components,
                ).update(
                    component_status=ComponentsStatus.objects.filter(
                        component_status_name="Under Maintenance"
                    ).first()
                )
            ...
            if recipients:
                for mail in recipients:
                    ...
                    if not sch_inc_emails:
                        create_recipients.append(
                            SchMntAdditionalRecipients(
                                email=mail, is_active=True, sch_inc=instance
                            )
                        )
                    else:
            ...
            SchMntAdditionalRecipients.objects.filter(
                Q(sch_inc=instance), ~Q(email__in=recipients)
...
class IncidentTemplateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if not businessunit_qs:
            raise ValidationError(
                {"Error": "Businessunit is not found or not in a active businessunit "}
            )
        try:
            instance.template_name = validated_data.get(
                "template_name", instance.template_name
            )
            instance.incident_title = validated_data.get(
                "incident_title", instance.incident_title
            )
